...
$sql = "SELECT f_id, f_title,f_image,description,price,quantity FROM menu";
$result = $con->query($sql);

if ($result->num_rows > 0) {
     // output data of each row

            echo "<table><tr><th>food_id</th><th>food_title</th><th>food_image</th><th>description</th><th>quantity</th><th>price</th></tr>";
     // output data of each row
     while($row = $result->fetch_assoc()) {
         echo "<tr><td>" . $row["f_id"]. "</td><td>" . $row["f_title"]. "</td><td> " . $row["f_image"]. "</td><td> " . $row["description"]."</td><td> " . $row["quantity"]."</td><td> " . $row["price"]."</td></tr>";
     }
     echo "</table>";
} else {
     echo "0 results";
